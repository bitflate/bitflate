...
#if defined(NDEBUG)
# error "Bitflate cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Bitflate Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (halvings > 1) {
        ...
        CAmount inflateCoins = round(105000000 * (pow(1.10, halvings - 2) - pow(1.10, halvings - 3)));
        // subsidy is inflateCoins / 210000 * COIN
        nSubsidy = round(((float)inflateCoins / (float)210000) * COIN);
    } else {
        nSubsidy = 500 * COIN;
...
void ThreadScriptCheck() {
    RenameThread("bitflate-scriptch");
